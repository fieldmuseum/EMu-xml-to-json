...
def get_ltc_values(ltc_node: dict, soup: BeautifulSoup) -> dict:
    ...
    for class_function in getmembers(ltc_classes):
        if isfunction(class_function[1]):
            class_function_list.append(class_function[0]) # working functions
            class_names_list.append(class_function[1]) # function names as text


    # Fill in terms for each class in the LTC scheme / more dynamically
    if ltc_record['ObjectGroup'] is not None:  # loop thru key,value in ltc_record
